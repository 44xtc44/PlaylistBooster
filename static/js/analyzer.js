...
function stageDrawAnalyzer(opt) {
  ...
  document.getElementById("rowCanvasRear").style.display = "none";
...
function disableAnalyzer(opt) {
  // show an image instead, img canvas sits behind analyzer
  if (animationFrameCount % 60 === 0) {
    let canvas = document.getElementById(opt.canvasId);
    let ctx = canvas.getContext('2d');
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    document.getElementById("rowCanvasRear").style.display = "block";
  }
}
